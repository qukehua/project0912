...
typedef struct {
    ...
    eth0_data_Query_SOFTWARE_VERSION*  original_request;   // 用于同步的条件变量
...
typedef struct {
    ...
    char errorInfo[256];    // 错误信息（若接口状态为失败，填入失败原因）
    bool response_ready;               // 响应就绪标志
     eth0_data_Query_REINITIATE* original_request;
...
typedef struct {
    ...
    char errorInfo[256];    // 错误信息（若接口状态为失败，填入失败原因）
    bool response_ready;               // 响应就绪标志
    eth0_data_Request_ROLLBACK* original_request;
...
typedef struct {
    ...
    char additionalInfo[8];   //若重启状态为失败，填入失败原因
    bool response_ready;               // 响应就绪标志
    eth0_data_Query_ROLLBACK* original_request;
